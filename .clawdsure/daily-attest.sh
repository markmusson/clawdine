...
# Step 1: Run ClawdStrike audit (if available)
...
# Step 1.5: Run threat model check
echo ""
echo "1️⃣.5️⃣  Running threat model verification..."
cd "$CLAWDSURE_DIR"

"$CLAWDSURE_DIR/scripts/threat-check.sh" >/dev/null 2>&1 || true

# Read results from saved report
THREAT_CHECK_JSON=$(cat "$CLAWDSURE_DIR/threat-check-report.json" 2>/dev/null || echo '{"threats_checked":0,"mitigated":0,"unmitigated":0,"critical_unmitigated":0,"high_unmitigated":0}')

# Parse threat check results
THREATS_CHECKED=$(echo "$THREAT_CHECK_JSON" | jq -r '.threats_checked // 0')
THREATS_MITIGATED=$(echo "$THREAT_CHECK_JSON" | jq -r '.mitigated // 0')
THREATS_UNMITIGATED=$(echo "$THREAT_CHECK_JSON" | jq -r '.unmitigated // 0')
CRITICAL_UNMITIGATED=$(echo "$THREAT_CHECK_JSON" | jq -r '.critical_unmitigated // 0')
HIGH_UNMITIGATED=$(echo "$THREAT_CHECK_JSON" | jq -r '.high_unmitigated // 0')

if [ "$CRITICAL_UNMITIGATED" -gt 0 ] || [ "$HIGH_UNMITIGATED" -gt 0 ]; then
  RESULT="FAIL"
  echo "   ⚠️  FAIL: Threat model violations detected"
else
  echo "   ✓ PASS"
fi
...
ATTESTATION=$(cat << EOF
{"seq":$SEQ,"prev":"$PREV_HASH","ts":"$TIMESTAMP","agent":"$AGENT_ID","result":"$RESULT","critical":$CRITICAL,"warn":$WARN,"info":$INFO,"version":"$VERSION","threats":{"checked":$THREATS_CHECKED,"mitigated":$THREATS_MITIGATED,"unmitigated":$THREATS_UNMITIGATED,"critical_unmitigated":$CRITICAL_UNMITIGATED,"high_unmitigated":$HIGH_UNMITIGATED}}
EOF
)
...
echo "Threats:    $THREATS_MITIGATED/$THREATS_CHECKED mitigated"
if [ "$CRITICAL_UNMITIGATED" -gt 0 ] || [ "$HIGH_UNMITIGATED" -gt 0 ]; then
  echo "            ⚠️  $CRITICAL_UNMITIGATED critical, $HIGH_UNMITIGATED high unmitigated"
fi
echo "CID:        $CID"
...
if [ "$RESULT" = "FAIL" ]; then
  ...
  echo "   Run: $CLAWDSURE_DIR/scripts/threat-check.sh"
  exit 1
fi
